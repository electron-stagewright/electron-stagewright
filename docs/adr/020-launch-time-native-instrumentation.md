...
1. synchronously runs a fixed, transport-owned hook (`TRAY_HOOK_BODY`) against `require('electron')`,
   patching `Tray` (a constructor wrapper plus `setToolTip` / `setTitle` / `setImage` / `setContextMenu`
   prototype patches and `destroy` cleanup) into a registry on
...
Because (1) runs before (2), the real main's `require('electron').Tray` resolves to the patched class even
when the app destructures `const { Tray } = require('electron')` at import time (the prototype patches
also catch a pre-wrap reference). The shim temp dir is removed when the session disposes (idempotent,
best-effort).
...
- **Opt-in per launch.** The operator passes `instrumentNative: true` deliberately; default off. Wrapping
  ...
  asked for it.
- **No agent code.** The hook (`TRAY_HOOK_BODY`) is a fixed transport-owned source string; the real-main
  path is the operator's own preflighted launch entry, JSON-escaped into a string literal and written to
