...
| App state altered by storage writes                                     | `storage_set_cookie` / `storage_clear_cookies` MODIFY app state (seed/clear a cookie), so they are bounded the same way: the `canAccessStorage` capability and a **first-party, operator-loaded plugin**; they run no app JavaScript and are not `--allow-eval` gated ([ADR-018](../adr/018-storage-plugin.md))                                                                                                                                                                                                                                                                                                                                     | A loaded plugin can seed or clear cookies; the operator chose to load it. No per-key `localStorage` / `IndexedDB` writes (that path needs renderer eval and is deferred)                                                                                                                                                                                                        |
...
| Native notification capture                                             | `native_notifications_*` OBSERVE the notifications the app shows by patching `Notification.prototype.show` in the main process (recording only the data fields — title/body/subtitle/silent/urgency — never handlers or refs), bounded by the `canAccessNativeUI` capability and a **first-party, operator-loaded plugin**; the agent supplies only arm/read/stop (no executable input), so although the fixed hook is installed via `evaluate` it is NOT `--allow-eval` gated ([ADR-019](../adr/019-native-ui-plugin.md))                                                                                                                          | An observe surface (user-facing notification text the app already displays); a loaded plugin can read shown notifications, the operator chose to load it. Playwright launch transport only; notifications shown before capture is armed are missed                                                                                                                              |
| App main entry wrapped by launch-time instrumentation                   | `electron_launch { main, instrumentNative: true }` (default OFF) wraps the app's main with a fixed, transport-owned hook that installs a `Tray` reader before the app runs, then loads the real main; the hook is a fixed source string (no agent code), the real-main path is the operator's own preflighted entry (JSON-escaped into a file Electron runs, never `eval`), and the shim is removed on stop ([ADR-020](../adr/020-launch-time-native-instrumentation.md))                                                                                                                                                                           | A launch-mechanism opt-in the operator sets per session (not implied by loading the plugin), bounding the shim's blast radius to opt-in sessions; executablePath-only launches cannot be instrumented; the wrapped main sees `process.argv[1]` pointing at the shim. Playwright launch transport only                                                                           |
