...
7. **Verify.**
   - npm: each package page shows the new version, then run `pnpm published:npx:smoke` from the
     ...
     could not parse a polluted stream) and passes the `node`, `playwright`, and `eval_policy` checks.
     It does **not** require the `electron` check: the smoke bootstraps into an isolated, cold
     Electron cache, which provisions no binary on some platforms, so requiring it would assert a
     property of that sandbox rather than of the published package. Driving a real Electron app is the
     e2e job's responsibility; it provisions the binary deterministically. The `Published npx smoke`
     workflow repeats this whenever a GitHub release is published.
...
| `electron_doctor` reports a failing `electron` check on a working setup               | fixed in core 0.4.1. Earlier versions required Electron's `path.txt`, which its install script writes but an install may skip even while the binary sits under `dist/` and launches fine                           | upgrade core, then confirm with an actual `electron_launch`                                                                                  |
| `electron_doctor` reports a failing `electron` check and `electron_launch` also fails | Electron's binary was never provisioned: nothing exists under the package's `dist/`. Whether an install provisions it varies by platform and cache state — a cold Linux runner may end up with the JS wrapper only | install Electron into the app project and launch with `--app-root` plus `runtime: "project"` (ADR-024), or pass an explicit `executablePath` |
