...
export function createStoragePlugin(): StagewrightPlugin {
    ...
    inputSchema: z
      .object({
        ...
        url: z
          .string()
          .optional()
          .describe('A URL the cookie applies to (set this OR domain+path).'),
        domain: z
          .string()
          .optional()
  ...
  function isIdbRecord(
    value: unknown,
  ): value is { readonly key: unknown; readonly value: unknown } {
    return (
  ...
  const idbStoreField = {
    ...
    store: z
      .string()
      .min(1)
      .describe('The object store name (must already exist in the database).'),
    ...sessionField,
  }
  ...
  const idbSchemaTool: AnyToolDefinition = defineTool({
    ...
    handler: async (args, ctx) => {
      ...
      if (args.database === undefined) {
        return makeSuccess(
          { origin: run.result.origin, databases: run.result.databases ?? [] },
          meta,
        )
      }
      return makeSuccess(
  ...
  const idbDeleteTool: AnyToolDefinition = defineTool({
    ...
    handler: async (args, ctx) => {
      ...
      if ('error' in run) return run.error
      return makeSuccess(
        { origin: run.result.origin, deleted: run.result.deleted ?? args.key },
        meta,
      )
    },
  })
...
export const storagePlugin: StagewrightPlugin = {
  ...
  coreVersionRange: '*',
  get tools() {
    return createStoragePlugin().tools ?? []
  },
  get errorCodes() {
    return createStoragePlugin().errorCodes ?? {}
  },
  get configSchema() {
    return configSchema
  },
