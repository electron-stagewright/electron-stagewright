...
// Capture state is created inside `createIpcPlugin`, keyed by the transport's globally-unique
// session id. Each server receives a fresh plugin instance, so co-resident servers cannot share
...
export const ipcPlugin: StagewrightPlugin = {
  ...
  coreVersionRange: '*',
  get tools() {
    return createIpcPlugin().tools ?? []
  },
  get errorCodes() {
    return createIpcPlugin().errorCodes ?? {}
  },
  get configSchema() {
    return configSchema
  },
